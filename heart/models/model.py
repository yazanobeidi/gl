...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        return self.w2(F.selu(self.w1(self.do(self.norm(x)))))
...
class Model(nn.Module):
    def __init__(self, input_shape, output_shape, dropout, ee_dim, name, seed, num_heads, common_embed_dim):
        ...
        self.layer1 = FC(input_shape[-1]*ee_dim, shape*4, dropout=dropout)
...
def load_models(training_sets, seed, personalize, ee_dim=16, num_heads=8, 
                common_embed_dim=8, dropout=0.0, device='cpu', name='heart'):
    if personalize:
